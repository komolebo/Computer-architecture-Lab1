...
def main(level):
    ...
    print "Searching all deep links ..."
    for lnk in get_references():
    ...
    for lnk in links_mp3:
        ...
        if not download_mp3(lnk, "song%d" % (i + 1)):
            links_mp3.remove(lnk)
            print "file not downloaded"
        i += 1
        if i == 1:  # Щоб довго не чекати
            break

    # Отримуємо інформацію із мета-даних
    i = 0
    while i < 1:  # len(links_mp3): # Щоб довго не чекати
        print "analysing " + "mp3s/song%d.mp3" % (i + 1)
        data.append(get_mp3_info("mp3s//song%d.mp3" % (i + 1)))
        i += 1
